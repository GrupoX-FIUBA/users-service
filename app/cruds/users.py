...
def create_user(db: Session, user: schemas.User):
    ...
    if(db_user is None):  # Si el usuario no esta cargado lo creo.
...
def follow(db: Session, user_id: str, user_id_to_follow: str):
    ...
    user_to_follow = db.query(models.User).filter(
        models.User.uid == user_id_to_follow).first()
    if (user_db is None or user_to_follow is None):
        raise Exception("User not Found")
...
def unfollow(db: Session, user_id: str, user_id_to_unfollow: str):
    ...
    user_to_unfollow = db.query(models.User).filter(
        models.User.uid == user_id_to_unfollow).first()
    if (user_db is None or user_to_unfollow is None):
        raise Exception("User not Found")
